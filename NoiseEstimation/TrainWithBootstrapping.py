...
def TrainModels(
        ...
        train_test_df, 
        extra_train_df,
        # organizer_pseudolabels,
        organizer_pseudolabels_weight,
        # extra_pseudolabels,
        extra_pseudolabels_weight,
        cat_params, 
    ...
    for fold_index, (train_index, test_index) in tqdm(enumerate(folds), total=fold_count):
        ...
        # MERGE WITH PSUEDOLABELS.
        extra_train_y = (extra_train_y * (1-extra_pseudolabels_weight)) + (extra_pseudolabels * extra_pseudolabels_weight)
        train_y = (train_y * (1-organizer_pseudolabels_weight)) + (organizer_pseudolabels * organizer_pseudolabels_weight)
        # train_y = (train_y * (1-organizer_pseudolabels_weight)) + (organizer_pseudolabels[train_index] * organizer_pseudolabels_weight)
...
def CreateEnsembleAndPredictions(
        ...
        fold_count, 
        starting_evals_json_paths,
        # organizer_pseudolabels_paths,
        organizer_pseudolabels_weight,
        # extra_pseudolabels_paths,
        extra_pseudolabels_weight,
    ...
    # organizer_pseudolabels = np.mean([
    #     pd.read_csv(filepath)['prediction'].values
    #     for filepath in organizer_pseudolabels_paths
    # ], axis=0)

    # extra_pseudolabels = np.mean([
    #     pd.read_csv(filepath)['prediction'].values
    #     for filepath in extra_pseudolabels_paths
    # ], axis=0)
...
if __name__ == '__main__':
    ...
    configs = [
        (0, 0.25, '00_025_v4_10fold_nested'),
        # (0, 0, '00_00_v4_10fold_nested'),
        # (0, 0.5, '02_05_v4_10fold_nested'),
        # (0.2, 0.5, '02_05_v4_10fold_nested'),
        # (0.3, 0.6, '03_06_v4_10fold_nested'),
        # (0.4, 0.7, '04_07_v4_10fold_nested'),
        # (0.5, 0.8, '05_08_v4_10fold_nested'),
        # (0.6, 0.9, '06_09_v4_10fold_nested'),
        # (0.3, 0.6, 'debug_04_06_v4_10fold'),
        # (0, 0, 'debug_00_00_v4_5fold'),
    ]

    for organizer_pseudolabels_weight, extra_pseudolabels_weight, output_directory_suffix in configs:
        ...
        for seed in [0]:
            ...
            rmse = CreateEnsembleAndPredictions(
                ...
                # organizer_pseudolabels_paths = glob.glob('NoiseEstimation/predictions/full_v4_organizer*.csv'),
                organizer_pseudolabels_weight = organizer_pseudolabels_weight,
                # extra_pseudolabels_paths = glob.glob('NoiseEstimation/predictions/full_v4_extra*.csv'),
                extra_pseudolabels_weight = extra_pseudolabels_weight,
                include_english_lsa = False,
                include_lud_lsa = False,
                oof_predictions_path = None,
